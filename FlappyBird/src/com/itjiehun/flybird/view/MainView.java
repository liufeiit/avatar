...
import com.itjiehun.flybird.object.Bird;
import com.itjiehun.flybird.object.Column;
	...
	public void drawSelf() {
		try {
			...
			if (isStart && !isHit && !isOver) {
				if (!isPause) {
					canvas.save();
					canvas.clipRect(pauseButtonImgX, pauseButtonImgY, pauseButtonImgX + pauseButtonImg.getWidth(),
					...
					canvas.drawBitmap(pauseButtonImg, pauseButtonImgX,
							pauseButtonImgY - pauseButtonImg.getHeight() / 2, paint);
					canvas.restore();
				}
			}
		} catch (Exception e) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN && event.getPointerCount() == 1) {
			...
			if (isWrite) {
				...
				if (x >= exitButtonImgX && x <= exitButtonImgX + exitButtonImg.getWidth() && y >= exitButtonImgY
