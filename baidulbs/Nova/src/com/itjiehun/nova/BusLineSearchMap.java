...
/**
 * 进行公交线路详情检索，并使用RouteOverlay在地图上绘制 同时展示如何浏览路线节点并弹出泡泡
 * 
 * @author 刘飞 E-mail:liufei_it@126.com
 * @version 1.0
 * @since 2014-6-20 下午6:14:59
 */
public class BusLineSearchMap extends FragmentActivity implements OnGetPoiSearchResultListener,
	...
	public void searchButtonProcess(View v) {
		...
		mSearch.searchInCity((new PoiCitySearchOption()).city(editCity.getText().toString()).keyword(
	...
	public void SearchNextBusline(View v) {
		...
		if (busLineIndex >= 0 && busLineIndex < busLineIDList.size() && busLineIDList.size() > 0) {
			mBusLineSearch.searchBusLine((new BusLineSearchOption().city(((EditText) findViewById(R.id.city)).getText()
	...
	public void nodeClick(View v) {
		...
		if (mBtnPre.equals(v) && nodeIndex > 0) {
			...
			mBaiduMap.showInfoWindow(new InfoWindow(popupText, route.getStations().get(nodeIndex).getLocation(), null));
		}
		// 下一个节点
	...
	public void onGetBusLineResult(BusLineResult result) {
		if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
			Toast.makeText(BusLineSearchMap.this, "抱歉，未找到结果", Toast.LENGTH_LONG).show();
			return;
		}
		mBaiduMap.clear();
		route = result;
		...
		Toast.makeText(BusLineSearchMap.this, result.getBusLineName(), Toast.LENGTH_SHORT).show();
	}
