 ...
 * @version 1.0
 * @since 2014-6-20 下午5:21:05
 */
public class GeoCoderMap extends Activity implements OnGetGeoCoderResultListener {
	...
	public void SearchButtonProcess(View v) {
		if (v.getId() == R.id.reversegeocode) {
			...
			LatLng ptCenter = new LatLng((Float.valueOf(lat.getText().toString())), (Float.valueOf(lon.getText()
					.toString())));
			// 反Geo搜索
			...
			// Geo搜索
			mSearch.geocode(new GeoCodeOption().city(editCity.getText().toString()).address(
	...
	public void onGetGeoCodeResult(GeoCodeResult result) {
		if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
			Toast.makeText(GeoCoderMap.this, "抱歉，未能找到结果", Toast.LENGTH_LONG).show();
		}
		mBaiduMap.clear();
	...
	public void onGetReverseGeoCodeResult(ReverseGeoCodeResult result) {
		...
		mBaiduMap.addOverlay(new MarkerOptions().position(result.getLocation()).icon(
				BitmapDescriptorFactory.fromResource(R.drawable.icon_marka)));
		mBaiduMap.setMapStatus(MapStatusUpdateFactory.newLatLng(result.getLocation()));
		Toast.makeText(GeoCoderMap.this, result.getAddress(), Toast.LENGTH_LONG).show();
	}
}